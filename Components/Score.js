...
export default function Score(props) {
	...
	useEffect(() => {
		props.socket.on("New Score", score => {
			setScoreGood(score.good);
			setScoreBad(score.bad);
		});
	}, []);
	...
	return (
		<TouchableOpacity
			onPress={() => setOpenScore(!openScore)}
			style={{ width: "100%" }}
		>
			<LinearGradient
			...
			>
				{openScore && <View style={{ height: 100 }}></View>}

				<View style={styles.scoreContainer}>
					<View>
						<Text style={[styles.score, { color: "#7ddbcc" }]}>
							{scoreGood}
						</Text>
					</View>
					...
					<View>
						<Text style={[styles.score, { color: "#eb9bba" }]}>
							{scoreBad}
						</Text>
					</View>
				</View>
